...
#define DEFAULT_TRIM_THRESHOLD (1024 * 1024 * 1024) /* (128 * 1024) EDB */
#endif
...
Void_t* public_mALLOc(size_t bytes) {
  ...
  if (bytes % sizeof(void *)) {
	  bytes = ((bytes / sizeof(void *)) + 1) * sizeof(void *);
  }
  m = mALLOc(bytes);
#ifdef INIT_MEM
  int i;
...
#if linux
      // Added by Emery Berger (www.cs.utexas.edu/users/emery)
      // Initially suggested by Jim Nance for Hoard (www.hoard.org).
      mm = (char*)(MMAP((void *) 0x10000000, size, PROT_READ|PROT_WRITE, MAP_PRIVATE));
#else
...
#if linux
      brk = (char*)(MMAP((void *) 0x10000000, size, PROT_READ|PROT_WRITE, MAP_PRIVATE));
#else
