...
value caml_ba_get_N(value vb, value * vind, int nind)
{
  ...
  case CAML_BA_SINT8:
    return Val_int(((caml_ba_int8 *) b->data)[offset]);
  case CAML_BA_UINT8:
    return Val_int(((caml_ba_uint8 *) b->data)[offset]);
  case CAML_BA_SINT16:
    return Val_int(((caml_ba_int16 *) b->data)[offset]);
  case CAML_BA_UINT16:
...
static value caml_ba_set_aux(value vb, value * vind, intnat nind, value newval)
{
  ...
  case CAML_BA_SINT8:
  case CAML_BA_UINT8:
    ((caml_ba_int8 *) b->data)[offset] = Int_val(newval); break;
  case CAML_BA_SINT16:
  case CAML_BA_UINT16:
    ((caml_ba_int16 *) b->data)[offset] = Int_val(newval); break;
  case CAML_BA_INT32:
                                        ...
                                        intnat min_val, intnat max_val)
{
  ...
  if (overflow_32) {
    ...
    for (n = 0, p = data; n < num_elts; n++, p++) caml_serialize_int_4((int32) *p);
  }
#else
  caml_serialize_int_1(0);
...
static void caml_ba_deserialize_longarray(void * dest, intnat num_elts)
{
  ...
  if (sixty) {
    ...
    intnat * p, n;
    for (n = 0, p = dest; n < num_elts; n++, p++) *p = caml_deserialize_sint_4();
  }
#else
  if (sixty)
