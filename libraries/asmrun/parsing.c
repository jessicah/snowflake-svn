...
static void print_token(struct parser_tables *tables, int state, value tok)
{
  ...
  if (Is_long(tok)) {
    ...
    v = Field(tok, 0);
    if (Is_long(v))
      dprintf("%ld", Long_val(v));
    else if (Tag_val(v) == String_tag)
      dprintf("%s", String_val(v));
    else if (Tag_val(v) == Double_tag)
      dprintf("%g", Double_val(v));
    else
      dprintf("_");
    dprintf(")\n");
  }          
                                 ...
                                 struct parser_env *env, value cmd, value arg)
{
  ...
  recover:
    if (errflag < 3) {
      ...
      while (1) {
        ...
        if (n1 != 0 && n2 >= 0 && n2 <= Int_val(tables->tablesize) &&
          ...
          if (caml_parser_trace) 
            dprintf("Recovering in state %d\n", state1);
          goto shift_recover;
        } else {
          if (caml_parser_trace){
            dprintf("Discarding state %d\n", state1);
          }
          if (sp <= Int_val(env->stackbase)) {
            if (caml_parser_trace){
              dprintf("No more states to discard\n");
            }
      ...
      if (Int_val(env->curr_char) == 0)
        return RAISE_PARSE_ERROR; /* The ML code raises Parse_error */
      if (caml_parser_trace) dprintf("Discarding last token read\n");
  ...
  shift_recover:
    if (caml_parser_trace)
      dprintf("State %d: shift to state %d\n",
  ...
  reduce:
    if (caml_parser_trace)
      dprintf("State %d: reduce by rule %d\n", state, n);
