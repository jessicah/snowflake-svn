...
let num_of_prim name =
  ...
  with Not_found ->
    if !Clflags.custom_runtime then
      ...
      let symb =
        try (*Dll.find_primitive name*) raise Not_found
  ...
  if String.length !Clflags.use_prims > 0 then begin
    ...
    try
      if (*Sys.command(Printf.sprintf "%s -p > %s"
                                    !Clflags.use_runtime primfile)*)1 <> 0
...
let get_global_value id =
  raise Not_found(*(Meta.global_data()).(slot_for_getglobal id)*)
let assign_global_value id v =
  raise Not_found(*(Meta.global_data()).(slot_for_getglobal id) <- v*)
