...
let openfile filename =
	begin try
		...
		let cookie = init_cookie (Bitstring.bitstring_of_string buffer) in
		...
		seek_in ic mdat.offset; (* ensure we're reading for reading from data offset *)
		Debug.printf "alac: opened file\n";
		(* done opening *)
		ic, blockio
	with ex ->
...
let size' = 14491
let buffer = String.make size' '\000'
let bitbuffer = BitBuffer.create buffer size'

let decode ic blockio =
	let size = (Int32.to_int !config.max_frame_bytes) + 1 in
	assert (size = size');

	let frame_length = Int32.to_int !config.frame_length in
	let chunk_size = frame_length * !config.num_channels * !config.bit_depth / 2 in

	(*(*let length = mdat.offset + mdat.size - 8 in*) these two values should be same? *)
...
while !continue do
	...
	let decoded = decode bitbuffer (Array1.sub blockio.data !offset chunk_size) frame_length !config.num_channels in
	let ok', decoded = match decoded with `ok n -> true, n | `fail (n,ex) ->
		Debug.printf "error decoding: %s\n" (Printexc.to_string ex);
		false, n
	in
	ok := ok';
	Debug.printf "+";
	...
	(* return if there's more data to consume *)
...
(*for i = blockio.pos to Array1.dim blockio.data - 1 do
	Debug.printf " %02x" blockio.data.{i};
if (i+1) mod 16 = 0 then Debug.printf "\n";
done;
Debug.printf "\n";*)
