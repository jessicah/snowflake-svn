...
(* $Id: optmain.ml,v 1.89.2.7 2007-06-15 15:09:54 frisch Exp $ *)
  ...
  try
    Arg.parse (Arch.command_line_options @ [
       ...
       "-dstartup", Arg.Set keep_startup_file, " (undocumented)";
			 "-freestanding", Arg.Unit(fun () -> free_standing := true; no_std_include := true), " (snowflake extension)";
			 "-use-runtime", Arg.String(fun s -> use_runtime := s), " (snowflake extension)";
			 "-clibrary", Arg.String(fun s -> c_libraries := !c_libraries ^ " " ^ s), " (snowflake extension)";
